...
from editor.editor_widget import NodeEditorWidget
from classes.vertex_base import *
from classes.edge import EDGE_TYPE_DIRECT
from graphics.view_actions import MODE_EDGE_DRAG#, MODE_EDGES_REROUTING
...
class CalculatorSubWindow(NodeEditorWidget):
    ...
    def showDialogLabel(self, edge):
        label, ok,  = QInputDialog.getText(self, 'Пометки дуги',
            'Буквенная пометка:')
        if ok:
            edge.edge_label = str(label)
            #edge.edge_bracket = str(bracket)

    def showDialogBracket(self, edge):
        bracket, ok,  = QInputDialog.getText(self, 'Пометки дуги',
            'Скобочная пометка:')
        if ok:
            edge.edge_bracket = str(bracket)
    ...
    def handleEdgeContextMenu(self, event):
        ...
        directAct = context_menu.addAction("Сделать дугу прямой")
        changeLabel = context_menu.addAction("Изменить буквенную пометку")
        changeBracket = context_menu.addAction("Изменить скобочную пометку")
        action = context_menu.exec_(self.mapToGlobal(event.pos()))
        ...
        if selected and action == changeLabel: self.showDialogLabel(selected)
        if selected and action == changeBracket: self.showDialogBracket(selected)
