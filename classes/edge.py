...
from graphics.edge_graphics import *
...
class Edge(Serializable):
    ...
    """
    def __init__(self, scene:'Scene', start_socket:'Socket'=None, end_socket:'Socket'=None, edge_type=EDGE_TYPE_DIRECT, edge_lbl='a', edge_bracket='(', edge_curves = 0):
        """
        ...
        self.edge_curve = len(self.start_socket.node.edges_in_self)  if start_socket is not None else edge_curves
        self.edge_label = edge_lbl
        self.edge_bracket = edge_bracket
#        self.text_pos_x = self.start_socket.pos().x()
 #       self.text_pos_y = self.start_socket.pos().y()
    ...
    def serialize(self) -> OrderedDict:
        return OrderedDict([
            ...
            ('curves', self.edge_curve),
            ('label', self.edge_label),
            ('bracket', self.edge_bracket)
        ])
    ...
    def deserialize(self, data:dict, hashmap:dict={}, restore_id:bool=True) -> bool:
        ...
        self.edge_curve = data['curves']
        self.edge_label = data['label']
        self.edge_bracket = data['bracket']
