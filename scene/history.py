...
class SceneHistory():
    ...
    def captureCurrentSelection(self) -> dict:
        """
        Create dictionary with list of selected vertices and list of selected edges
        :return: ``dict`` 'vertices' - list of selected vertices, 'edges' - list of selected edges
        :rtype: ``dict``
        """
        sel_obj = {
            'vertices': [],
        ...
        for item in self.scene.grScene.selectedItems():
            if hasattr(item, 'node'): sel_obj['vertices'].append(item.node.id)
    ...
    def restoreHistoryStamp(self, history_stamp:dict):
        ...
        try:
            ...
            if DEBUG_SELECTION: print("selected vertices before restore:", previous_selection['vertices'])
            ...
            # first clear all selection on vertices
            for node in self.scene.nodes: node.grNode.setSelected(False)
            # now restore selected vertices from history_stamp
            for node_id in history_stamp['selection']['vertices']:
            ...
            if DEBUG_SELECTION: print("selected vertices after restore:", current_selection['vertices'])

            # if the selection of vertices differ before and after restoration, set flag
            if current_selection['vertices'] != previous_selection['vertices'] or current_selection['edges'] != previous_selection['edges']:
